"""Validate the native CLRBHB package in one disposable Binary Ninja GUI profile."""
...
from build_arm64 import install
...
def main():
    ...
    if not all((args.reference, args.target, args.bindiff)):
        parser.error("acceptance requires reference, target and bindiff")
    ...
    with lock_path.open("a") as lock:
        ...
        if any(Path(line.strip()).name == "binaryninja" for line in listing.stdout.splitlines()):
            parser.error("an existing Binary Ninja GUI must finish before this isolated probe")
        ...
        install(args.package, profile, args.binaryninja.parent.parent.parent)
        config = {
            "replacement": str(profile / "plugins/libarch_arm64.dylib"),
            "case": args.case,
            "output": str(root),
        ...
        time.sleep(2)
        crashes = sorted(p.name for p in set(crash_dir.glob("binaryninja*")) - before_crashes)
        capture_path = root / "result.json"
        if args.case in ("quit", "quit_matching") and (root / "capture/result.json").exists():
