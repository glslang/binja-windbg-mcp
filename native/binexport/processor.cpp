// Based on Binary Ninja SDK 2ddf304b3275aa184e95570404539cbc4beb64c6:
// https://github.com/Vector35/binaryninja-api/blob/2ddf304b3275aa184e95570404539cbc4beb64c6/plugins/bindiff_similarity/processor.cpp
// Local change: export exact aligned AArch64 CLRBHB instructions rejected by BN6's decoder.
#include "processor.h"
#include "clrbhb.h"
...
void BinDiffProcessor::AddFunction(BinaryNinja::Function& func)
{
	...
	for (const auto& block : basicBlocks)
	{
		...
		while (instrAddr < block->GetEnd())
		{
			...
			if (bufferLen == 0)
				break;
			const bool decoded = blockArch->GetInstructionInfo(instrData, instrAddr, bufferLen, instrInfo);
			const auto clrbhb = !decoded && block->GetEnd() - instrAddr >= 4
				? GetAarch64ClrBhbFallback(blockArch->GetName(), instrAddr, instrData, bufferLen)
				: std::nullopt;
			if (!decoded && !clrbhb)
				break;
			if (clrbhb)
			{
				instrInfo = {};
				instrInfo.length = clrbhb->size;
			}
			instrTokens.clear();
			if (decoded && !blockArch->GetInstructionText(instrData, instrAddr, instrInfo.length, instrTokens))
				break;

			Instruction instrBd = clrbhb ? Instruction(instrAddr, instrAddr + clrbhb->size, clrbhb->size,
				std::string(clrbhb->mnemonic), {}) : ParseInstructionBinaryNinja(instrAddr, instrInfo, instrTokens);
